...
public class test {
	...
	public void BoardGameTest() {
		...
		Assert.assertEquals("[player1, player2]", game.getPlayers().toString());
		Assert.assertEquals("[LIBRARY, STUDY]", game.getPlayerLocations().toString());
		Assert.assertEquals("[MAGENTA_RACER: a MAGENTA racecar with priority 1, YELLOW_BOOT: a YELLOW boot with priority 7]", 
				game.getPlayerPieces().toString());
		
		Assert.assertEquals("[MAGENTA_RACER: a MAGENTA racecar with priority 1]", game.getGamePiecesAtLocation(Location.LIBRARY).toString());
	}
